...
if __name__ == '__main__':
    ...
    from ursina import *
    app = Ursina()
    wp = WindowPanel(title='Custom Window',content=(Text('Name:'),InputField(name='name_field'),Button(text='Submit', color=color.azure),Slider(),Slider(),ButtonGroup(('test', 'eslk', 'skffk'))),enabled = True)
    invoke(wp.enable,delay = 8)
    invoke(wp.disable,delay = 10)
    wp.y = wp.panel.scale_y / 2 * wp.scale_y    # center the window panel
    invoke(wp.disable,delay = 5)
    app.run()
